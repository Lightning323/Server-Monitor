...
case "custom-command":
        ...
        button.onclick = function(){
            let userConfirmed = confirm("Do you want to run custom command \""+data+"\"?");
            if (userConfirmed) {
                $('#custom-command-output').html('');
                socket.send("custom-command"+DELIMITER+data);
            }
        };
        ...
        break;
case "custom-command-output":
        console.log(data);
        $("#custom-command-output").text(
        ($("#custom-command-output").text() +" \n"+ data));
        //Scroll to bottom
        $("#custom-command-scroll").scrollTop($("#custom-command-scroll")[0].scrollHeight);
        break;
case "custom-command-exit":
        $("#custom-command-output").text(
        ($("#custom-command-output").text() +" \nExit code: "+ data));
        //Scroll to bottom
        $("#custom-command-scroll").scrollTop($("#custom-command-scroll")[0].scrollHeight);
